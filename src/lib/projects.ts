    ...
    sections: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            accent: true,
            bullets: [
              "Java 동적 매개변수 기반 화면별 쿼리를 마트로 전환, 동일 구조 화면은 통합 재설계",
        ...
        rows: [
          {
            ...
            action:
              "동일 구조 화면은 통합 마트로 재설계하고, 동적 매개변수를 제거하여 필터 대상 필드를 SELECT 절에 노출, PK·인덱스 표준화 및 프로시저별 정합성 검증 체계 구축",
            result:
    ...
    sections: [
...

      {
        ...
        rows: [
          {
            problem:
              "Excel 피벗 기반 수기 보고로 분석 자유도가 낮고 부서별 자율 분석이 어려움",
            action:
              "Tableau 기반 목적별 대시보드로 전환, 연월 필터 및 PDF·PPT 다운로드 전 화면 구현",
            result:
              "경영·마케팅·현장 각 목적에 맞는 보고 체계 구축 + 셀프 BI 환경 구축 및 교육 지원으로 분석 자율성 확보",
          },
          {
            problem:
              "전국 매장 수가 3천~5천개 이상으로, Tableau 퀵 필터에 그대로 노출할 경우 조회 성능이 크게 저하",
            action:
              "매장 필터를 팝업 기반 사전 필터링 구조로 전환",
            result:
              "대량 매장 데이터에서도 안정적인 조회 성능 확보, 향후 매장 수가 늘어나도 성능 저하 없이 확장 가능한 구조 마련",
          },
          {
            problem:
              "대시보드 로딩 목표 7초 이내 충족을 위해, 12개월 전 대비 환급건수·판매금액 비교 지표를 대시보드 내에서 직접 집계하면 성능 저하 우려",
            action:
              "12개월 전 환급건수·판매금액·방문객수를 사전 집계 컬럼으로 포함한 마트 구조를 설계하여 요청, 대시보드 내 실시간 연산 최소화",
            result:
              "대시보드 내 연산을 최소화하여 목표 로딩 시간(7초 이내) 달성",
          },
        ],
      },
      {
    ...
    sections: [
...

      {
        ...
        rows: [
          {
            problem:
              "각 마트의 행 수가 많고 목적별로 테이블이 분리되어 있어, DB에서 물리적으로 조인하면 행 수가 기하급수적으로 증가 — 그럼에도 대시보드 내 모든 차트가 서로 필터 연동되어야 하는 요구사항",
            action:
              "회원 번호·주문 번호를 기준으로 Tableau '관계'를 설정하여, 테이블을 물리적으로 합치지 않고도 필터·드릴다운이 전파되는 구조 구현",
            result:
